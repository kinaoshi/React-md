...
1. src/components/Posts/index.jsx をスタイリング
...
4. posts の中の post のスタイリング

- src/Components/post/index.jsx の
  > ＜/Head ＞の下を
...
- ul タグの"grid-cols-3"を"に"gap-4 を gap-6"に変更
- a タグの p-2 を p-4 に
- h1 タグに"text-xl truncate"を追加
- div タグの"text-sm を"text-lg truncate"に変更
  > src/Comments/Comments/index.jsx についても
- a タグの text-sm を削除
  > post のコメント一覧の下にもう少し余白をつける
...
- - h2 タグ"投稿"は"className="text-xl font-bold mt-10""
- li タグは
...
> とする
> PostsByUserId は選択して、呼び出し元(PostsByUserId.jsx)に飛んで
...
- Comments/Comment/index.jsx の中の
